...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        if cfg.backbone == 'swin_s':
            ...
            from .encoders.dual_segformer import mit_b0 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b5_w_aspp':
            logger.info('Using backbone: Segformer-B5 with ASPP')
            from .encoders.dual_segformer_w_aspp import mit_b5 as backbone
            self.channels = [96, 192, 384, 768]
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b4_w_aspp':
            logger.info('Using backbone: Segformer-B4 with ASPP')
            from .encoders.dual_segformer_w_aspp import mit_b4 as backbone
            self.channels = [96, 192, 384, 768]
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b3_w_aspp':
            logger.info('Using backbone: Segformer-B3 with ASPP')
            from .encoders.dual_segformer_w_aspp import mit_b3 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b2_w_aspp':
            logger.info('Using backbone: Segformer-B2 with ASPP')
            from .encoders.dual_segformer_w_aspp import mit_b2 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b1_w_aspp':
            logger.info('Using backbone: Segformer-B1 with ASPP')
            from .encoders.dual_segformer_w_aspp import mit_b0 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b0_w_aspp':
            logger.info('Using backbone: Segformer-B0 with ASPP')
            self.channels = [32, 64, 160, 256]
            from .encoders.dual_segformer_w_aspp import mit_b0 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
