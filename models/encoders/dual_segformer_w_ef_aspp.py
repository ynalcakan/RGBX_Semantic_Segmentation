...
class eASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, norm_layer=nn.BatchNorm2d):
        ...
        # Define dimensions based on the diagram
        self.reduce_dim = 64
        self.middle_dim = 256
        self.output_dim = in_channels  # Keep output same as input
        
        # Input 1x1 conv at the top of diagram
        self.input_conv = nn.Sequential(
            nn.Conv2d(in_channels, self.middle_dim, 1, bias=False),
            norm_layer(self.middle_dim),
            nn.ReLU(True)
        )
        
        # Create the three parallel branches
        rate1, rate2, rate3 = tuple(atrous_rates)
        
        # Branch 1 (top row in diagram)
        self.branch1 = nn.ModuleList([
            nn.Sequential(  # 1x1 conv
                nn.Conv2d(in_channels, self.reduce_dim, 1, bias=False),
                norm_layer(self.reduce_dim),
                nn.ReLU(True)
            ),
            ASPPConv(self.reduce_dim, self.reduce_dim, rate1, norm_layer),  # 3x3 rate1
            ASPPConv(self.reduce_dim, self.reduce_dim, rate1, norm_layer),  # 3x3 rate1
            ASPPConv(self.reduce_dim, self.reduce_dim, rate1, norm_layer),  # 3x3 rate1
            nn.Sequential(  # 1x1 conv to middle_dim
                nn.Conv2d(self.reduce_dim, self.middle_dim, 1, bias=False),
                norm_layer(self.middle_dim),
                nn.ReLU(True)
            )
        ])
        
        # Branch 2 (middle row in diagram)
        self.branch2 = nn.ModuleList([
            ...
            ASPPConv(self.reduce_dim, self.reduce_dim, rate2, norm_layer),  # 3x3 rate2
            ASPPConv(self.reduce_dim, self.reduce_dim, rate2, norm_layer),  # 3x3 rate2
            ASPPConv(self.reduce_dim, self.reduce_dim, rate2, norm_layer),  # 3x3 rate2
            nn.Sequential(  # 1x1 conv to middle_dim
                nn.Conv2d(self.reduce_dim, self.middle_dim, 1, bias=False),
        ...
        # Branch 3 (bottom row in diagram)
        self.branch3 = nn.ModuleList([
            ...
            ASPPConv(self.reduce_dim, self.reduce_dim, rate3, norm_layer),  # 3x3 rate3
            ASPPConv(self.reduce_dim, self.reduce_dim, rate3, norm_layer),  # 3x3 rate3
            ASPPConv(self.reduce_dim, self.reduce_dim, rate3, norm_layer),  # 3x3 rate3
            nn.Sequential(  # 1x1 conv to middle_dim
                nn.Conv2d(self.reduce_dim, self.middle_dim, 1, bias=False),
        ...
        # Image Pooling branch (bottom of diagram)
        self.img_pooling = AsppPooling(in_channels, self.middle_dim, norm_layer)
        
        # Concat and final 1x1 projection
        self.project = nn.Sequential(
            nn.Conv2d(self.middle_dim * 5, self.output_dim, 1, bias=False),
    ...
    def forward(self, x):
        ...
        # Process branch 3
        b3 = x
        for module in self.branch3:
            b3 = module(b3)
        
        # Image pooling
        img_pool = self.img_pooling(x)
        
        # Concatenate all features
        concat = torch.cat([x_in, b1, b2, b3, img_pool], dim=1)
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        # Replace multiple eASPP modules with a single one at the end
        # self.aspp_modules = nn.ModuleList([
        #     eASPP(embed_dims[0], atrous_rates=[3, 6, 9], norm_layer=nn.BatchNorm2d),
        #     eASPP(embed_dims[1], atrous_rates=[6, 12, 18], norm_layer=nn.BatchNorm2d),
        #     eASPP(embed_dims[2], atrous_rates=[12, 24, 36], norm_layer=nn.BatchNorm2d),
        #     eASPP(embed_dims[3], atrous_rates=[12, 24, 36], norm_layer=nn.BatchNorm2d)
        # ])
        
        # Single eASPP at the end with larger atrous rates
        self.single_aspp = eASPP(embed_dims[3], atrous_rates=[12, 24, 36], norm_layer=nn.BatchNorm2d)
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        # Apply single eASPP at the final stage
        x_fused = self.single_aspp(x_fused)
        outs.append(x_fused)
